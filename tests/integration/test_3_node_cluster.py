...
class TestThreeNodeLockCluster:
    ...
    async def test_distributed_lock_across_nodes(self, three_node_cluster):
        ...
        success1 = await nodes[0].acquire_lock(resource, nodes[0].node_id, exclusive=True)
        assert success1, "Node 1 should acquire lock"
        
        # Node 2 tries to acquire same lock (should fail)
        success2 = await nodes[1].acquire_lock(resource, nodes[1].node_id, exclusive=True)
        ...
        success3 = await nodes[1].acquire_lock(resource, nodes[1].node_id, exclusive=True)
    ...
    async def test_shared_locks_multiple_nodes(self, three_node_cluster):
        ...
        for i, node in enumerate(nodes):
            success = await node.acquire_lock(resource, node.node_id, exclusive=False)
    ...
    async def test_leader_failure_recovery(self, three_node_cluster):
        ...
        success = await leader.acquire_lock(resource, leader.node_id, exclusive=True)
        assert success
    ...
    async def test_deadlock_detection_multi_node(self, three_node_cluster):
        ...
        # Node1 acquires A
        await nodes[0].acquire_lock("resource_A", "node1", exclusive=True)
        await asyncio.sleep(0.3)
        
        # Node2 acquires B
        await nodes[1].acquire_lock("resource_B", "node2", exclusive=True)
        await asyncio.sleep(0.3)
        
        # Node3 acquires C
        await nodes[2].acquire_lock("resource_C", "node3", exclusive=True)
        await asyncio.sleep(0.3)
...
class TestNetworkPartition:
    ...
    async def test_partition_detection(self, three_node_cluster):
        ...
        # Simulate partition by stopping message passing for one node
        follower = next(n for n in nodes if n.raft.state == 'follower')
