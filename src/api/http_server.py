...
class HTTPAPIServer:
    ...
    def _create_app(self):
        ...
        for route in list(app.router.routes()):
            ...
            if not any(
                r.method == "OPTIONS" and r.resource.canonical == route_info
    ...
    async def _error_middleware(self, request, handler):
        ...
        try:
            ...
            response = web.json_response(
                {"error": str(e), "status": e.status, "path": request.path},
                status=e.status,
            )
            response.headers["Access-Control-Allow-Origin"] = "*"
            return response
            ...
            response = web.json_response(
                {"error": "Request cancelled", "status": 499, "path": request.path},
                status=499,
            )
            response.headers["Access-Control-Allow-Origin"] = "*"
            return response
            ...
            logger.error(
                f"Unexpected error handling {request.method} {request.path}: {e}"
            )
            logger.exception(e)
            response = web.json_response(
                {
    ...
    async def start(self):
        ...
        try:
            logger.info(
                f"Starting HTTP API server for {self.node.node_id} on {self.host}:{self.port}"
            )
            ...
            # Force binding to 0.0.0.0 so published host ports are always reachable
            # from the Docker host. This avoids cases where API_HOST is set to
            # localhost/127.0.0.1 inside the container, which prevents external
            # connections from reaching the service even when the port is published.
            bind_host = "0.0.0.0"
            logger.info(
                "Forcing HTTP bind to %s (published host ports will be reachable)",
                bind_host,
            )
            site = web.TCPSite(
                runner,
            ...
            logger.info(
                f"HTTP API server for {self.node.node_id} started successfully on {self.host}:{self.port}"
            )
            return True
    ...
    async def handle_status(self, request):
        ...
        try:
            ...
            logger.exception(e)
            return web.json_response(
                {"error": "Internal server error", "status": "error"}, status=500
            )
    ...
    async def handle_lock_acquire(self, request):
        ...
        try:
            ...
            if not resource or not client_id:
                return web.json_response(
                    {"success": False, "error": "Missing resource or client_id"},
                    status=400,
                )

            if not hasattr(self.node, "acquire_lock"):
                return web.json_response(
                    {"success": False, "error": "Lock manager not available"},
                    status=400,
                )
    ...
    async def handle_lock_release(self, request):
        ...
        try:
            ...
            return web.json_response(
                {"success": success, "resource": resource, "client_id": client_id}
            )
    ...
    async def handle_lock_status(self, request):
        ...
        try:
            if not hasattr(self.node, "get_lock_status"):
                return web.json_response(
                    {"error": "Lock manager not available"}, status=400
                )
