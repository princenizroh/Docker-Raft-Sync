...
class ClusterManager:
    ...
    def __init__(self, node_type: str = "lock"):
        ...
        self.node_configs = [
            ...
            {"node_id": "node-2", "host": "127.0.0.1", "port": 5002, "api_port": 6010},
            {"node_id": "node-3", "host": "127.0.0.1", "port": 5003, "api_port": 6020},
        ]
        ...
        if node_type == "lock":
            ...
            for config in self.node_configs:
                ...
                config["http_port"] = config[
                    "api_port"
                ]  # Store HTTP API port separately
    ...
    async def start_nodes(self):
        ...
        for config in self.node_configs:
            if self.node_type == "lock":
                ...
                # Create the DistributedLockManager without auto-initializing the HTTP API,
                # then attach an HTTPAPIServer bound to the configured api_port to avoid
                # multiple nodes trying to bind the same default port.
                from src.nodes.lock_manager import DistributedLockManager
                ...
                node = DistributedLockManager(
                    ...
                    enable_http_api=False,  # Prevent BaseNode from creating a default HTTP server
                )
                # Explicitly attach HTTP server with the configured API port to avoid collisions
                node.http_server = HTTPAPIServer(
                    node, config["host"], config["api_port"]
                )
                ...
                node.http_server = HTTPAPIServer(
                    node, config["host"], config["port"] + 1000
                )
                node.enable_http_api = True
            ...
            try:
                ...
                if hasattr(node, "http_server") and node.http_server:
                    try:
                        ...
                        logger.error(
                            f"Error stopping HTTP server for {node.node_id}: {e}"
                        )
    ...
    async def _check_docker_cluster_health(self):
        ...
        try:
            ...
            if healthy < len(self.nodes):
                logger.warning(
                    f"Docker cluster health: {healthy}/{len(self.nodes)} nodes responding"
                )
...
async def main():
    ...
    # Check Docker containers first (can be skipped via SKIP_DOCKER_CHECK env var)
    # Set SKIP_DOCKER_CHECK=1 (or true) to bypass Docker presence verification when running nodes locally.
    if os.environ.get("SKIP_DOCKER_CHECK") in ("1", "true", "True", "TRUE"):
        print("⚠ SKIP_DOCKER_CHECK is set; skipping Docker container check")
    else:
        try:
