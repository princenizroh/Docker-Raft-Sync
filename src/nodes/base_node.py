...
class BaseNode:
    ...
    def __init__(
        ...
        self.enable_http_api = (
            enable_http_api
            if enable_http_api is not None
            else config.api.enable_http_api
        )
        logger.info(
            f"HTTP API enabled: {self.enable_http_api} (explicit={enable_http_api}, config={config.api.enable_http_api})"
        )
        ...
        if enable_http_api:
            ...
            try:
                ...
                # Get API settings from config/env if provided; fall back to computed defaults
                # Prefer explicit configuration (eg. API_HOST/API_PORT env vars) so that the
                # runtime can control binding and published ports in containerized setups.
                api_host = (
                    getattr(config.api, "api_host", None) or config.api.api_host
                    if getattr(config, "api", None)
                    else None
                )
                api_port = (
                    getattr(config.api, "api_port", None)
                    if getattr(config, "api", None)
                    else None
                )

                # If config did not provide a port, fall back to the legacy per-node calculation
                if not api_port:
                    try:
                        idx = (
                            int(str(self.node_id).split("-")[1]) - 1
                        )  # node-1 -> 0, node-2 -> 1, ...
                        api_port = 6000 + (idx * 10)
                    except Exception:
                        api_port = 6000

                # If config did not provide a host, default to 0.0.0.0 so published container ports work
                if not api_host:
                    api_host = (
                        getattr(config.api, "api_host", None)
                        if getattr(config, "api", None)
                        else None
                    )
                if not api_host:
                ...
                self.http_server = HTTPAPIServer(self, api_host, int(api_port))
                logger.info(
                    f"HTTP API server initialized at http://{api_host}:{api_port}"
                )
    ...
    def _setup_message_handlers(self):
        ...
        self.message_passing.register_handler(
            MessageType.REQUEST_VOTE.value, self._handle_request_vote
        )
        self.message_passing.register_handler(
            MessageType.VOTE_RESPONSE.value, self._handle_vote_response
        )
        self.message_passing.register_handler(
            MessageType.APPEND_ENTRIES.value, self._handle_append_entries
        )
        self.message_passing.register_handler(
        ...
        self.message_passing.register_handler(MessageType.PING.value, self._handle_ping)
        self.message_passing.register_handler(MessageType.PONG.value, self._handle_pong)
    ...
    async def start(self):
        ...
        if self.http_server:
            ...
            try:
                ...
                logger.info(
                    f"HTTP API server is now listening at http://{self.http_server.host}:{self.http_server.port}"
                )
            except Exception as e:
    ...
    async def _handle_vote_response(self, message: Message):
        ...
        await self.raft.handle_vote_response(
            {**message.payload, "term": message.term, "sender_id": message.sender_id}
        )
    ...
    async def _handle_append_entries(self, message: Message):
        ...
        with PerformanceTimer(self.metrics, "append_entries_handling_time"):
            await self.raft.handle_append_entries(
                {**message.payload, "term": message.term}
            )
...
async def main():
    ...
    parser.add_argument(
        "--cluster-nodes",
        required=True,
        help="Comma-separated list of cluster nodes (id:host:port)",
    )
    parser.add_argument(
        "--enable-http-api", action="store_true", help="Enable HTTP API server"
    )
