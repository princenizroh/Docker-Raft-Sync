...
async def test_log_append_throughput():
    ...
    try:
        ...
        throughput = num_operations / max(duration, 0.001)  # Avoid division by zero
...
async def test_election_timeout_accuracy():
    """Test election timeout is within expected range"""
    ...
    # Should have triggered election (become candidate)
...
async def test_concurrent_log_appends():
    """Test concurrent log append performance"""
    node = RaftNode('perf-node', [])
    ...
    try:
        ...
        # Run 5 concurrent batches of 20 operations each
        start_time = time.time()
        ...
        throughput = 100 / max(duration, 0.001)  # Avoid division by zero
        
        # Should handle concurrent appends
...
async def test_multiple_nodes_startup_time():
    """Test multi-node cluster startup performance"""
    nodes = [
...
async def test_log_retrieval_performance():
    """Test log status retrieval performance"""
    node = RaftNode('perf-node', [])
    ...
    try:
        ...
        ops_per_sec = 1000 / max(duration, 0.001)  # Avoid division by zero
...
def test_memory_footprint():
    """Test that nodes have reasonable memory footprint"""
    import sys
    ...
    # Should be reasonable (< 10KB for basic node)
