...
async def benchmark_raft_consensus(
    results: BenchmarkResults, num_operations: int = 1000
):
    ...
    nodes = [
        RaftNode(
            "node-1",
            ["node-1:localhost:6000", "node-2:localhost:6010", "node-3:localhost:6020"],
        ),
        RaftNode(
            "node-2",
            ["node-1:localhost:6000", "node-2:localhost:6010", "node-3:localhost:6020"],
...
async def benchmark_distributed_locks(
    results: BenchmarkResults, num_operations: int = 500
):
    ...
    config = NodeConfig(
        ...
        port=6000,
        cluster_nodes=[
            "node-1:localhost:6000",
            "node-2:localhost:6010",
            "node-3:localhost:6020",
        ],
    )
    ...
    try:
        ...
        for i in range(num_operations):
            ...
            lock_id = await lock_manager.acquire_lock(
                resource_id=resource_id, lock_type=LockType.EXCLUSIVE, timeout=5.0
            )
...
async def benchmark_distributed_queue(
    results: BenchmarkResults, num_messages: int = 1000
):
    ...
    try:
        ...
        results.add_result(
            "Distributed Queue", "Enqueue Throughput (msg/sec)", enqueue_throughput
        )
        results.add_result(
            "Distributed Queue",
            "Avg Enqueue Latency (ms)",
            (enqueue_duration / num_messages) * 1000,
        )
        results.add_result("Distributed Queue", "Messages Dequeued", dequeued)
        results.add_result(
            "Distributed Queue", "Dequeue Throughput (msg/sec)", dequeue_throughput
        )
        results.add_result(
            "Distributed Queue",
            "Avg Dequeue Latency (ms)",
            (dequeue_duration / dequeued) * 1000 if dequeued > 0 else 0,
        )
        results.add_result(
            "Distributed Queue",
            "Message Loss Rate (%)",
            ((num_messages - dequeued) / num_messages) * 100,
        )
...
async def benchmark_distributed_cache(
    ...
    try:
        ...
        results.add_result(
            "Distributed Cache", "PUT Throughput (ops/sec)", put_throughput
        )
        ...
        results.add_result(
            "Distributed Cache", "GET Throughput (ops/sec)", get_throughput
        )
