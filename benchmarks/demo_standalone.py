...
Demonstrates system capabilities with independent nodes
"""
...
async def demo_queue(node: DistributedQueue):
    ...
    for i in range(5):
        success = await node.enqueue('test-queue', f"Message {i}: Hello from distributed queue!")
        print(f"   Enqueued: Message {i}: Hello from distributed queue! - {'SUCCESS' if success else 'FAILED'}")
